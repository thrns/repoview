...
export function Sidebar({ className, ...props }: HTMLAttributes<HTMLElement>) { return <aside className={cn('hidden h-full min-h-0 w-60 shrink-0 border-r border-border/70 bg-background lg:flex lg:flex-col', className)} {...props} /> }
export function SidebarHeader({ className, ...props }: HTMLAttributes<HTMLDivElement>) { return <div className={cn('p-4', className)} {...props} /> }
export function SidebarContent({ className, ...props }: HTMLAttributes<HTMLElement>) { return <nav className={cn('flex-1 overflow-auto px-4 py-5', className)} aria-label="Primary navigation" {...props} /> }
