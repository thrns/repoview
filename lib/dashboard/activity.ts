...
export async function getDashboardActivity(filter: ActivityFilter = 'all'): Promise<DashboardActivityItem[]> {
  ...
  const [{ data: events, error: eventsError }, { data: notifications, error: notificationsError }] = await Promise.all([
    admin.from('view_events').select('*').order('created_at', { ascending: false }).limit(100),
    admin.from('notification_deliveries').select('*').order('created_at', { ascending: false }).limit(100),
  ])
  ...
  const resolvedShares = shareIds.size > 0
    ? await admin.from('shares').select('id, recipient_label, repository_id').in('id', [...shareIds])
    : { data: [], error: null }
  ...
  const resolvedRepositories = repositoryIds.length > 0
    ? await admin.from('repositories').select('id, github_owner, github_repo').in('id', repositoryIds)
    : { data: [], error: null }
  ...
  const resolvedSessions = sessionIds.length > 0
    ? await admin.from('viewer_sessions').select('id, viewer_id, browser, device_type, country, first_seen_at, last_seen_at, ended_at, active_ms, idle_ms').in('id', sessionIds)
    : { data: [], error: null }
  ...
  const resolvedViewers = viewerIds.length > 0
    ? await admin.from('viewers').select('id, viewer_code').in('id', viewerIds)
    : { data: [], error: null }
