...
export async function listViewerDashboardItems(): Promise<ViewerDashboardItem[]> {
  ...
  const [{ data: viewers, error: viewersError }, { data: sessions, error: sessionsError }, { data: shares, error: sharesError }, { data: recipients, error: recipientsError }, { data: repositories, error: repositoriesError }, { data: engagement, error: engagementError }] = await Promise.all([
    admin.from('viewers').select('*').order('last_seen_at', { ascending: false }),
    admin.from('viewer_sessions').select('*').not('confirmed_at', 'is', null),
    admin.from('shares').select('*'),
    admin.from('share_recipients').select('*'),
    admin.from('repositories').select('*'),
    admin.from('file_engagement').select('viewer_id, path, active_ms'),
  ])
...
export async function getViewerDetail(viewerId: string): Promise<ViewerDetailData> {
  const admin = createSupabaseAdminClient()
  const { data: viewer, error: viewerError } = await admin.from('viewers').select('*').eq('id', viewerId).maybeSingle()
  ...
  const [{ data: sessions, error: sessionsError }, { data: shares, error: sharesError }, { data: recipients, error: recipientsError }, { data: repositories, error: repositoriesError }, { data: events, error: eventsError }, { data: engagements, error: engagementError }] = await Promise.all([
    admin.from('viewer_sessions').select('*').eq('viewer_id', viewerId).order('first_seen_at', { ascending: false }),
    admin.from('shares').select('*'),
    admin.from('share_recipients').select('*'),
    admin.from('repositories').select('*'),
    admin.from('view_events').select('*').in('session_id', await getSessionIds(admin, viewerId)).order('created_at', { ascending: true }),
    admin.from('file_engagement').select('*').eq('viewer_id', viewerId).order('last_viewed_at', { ascending: false }),
  ])
...
async function getSessionIds(admin: ReturnType<typeof createSupabaseAdminClient>, viewerId: string) {
  const { data } = await admin.from('viewer_sessions').select('id').eq('viewer_id', viewerId)
